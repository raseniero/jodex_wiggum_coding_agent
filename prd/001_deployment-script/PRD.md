...
**Success Metric:** Script deploys all target `.md` files with 0 exit code, 0 missing files, and completes in <5 seconds
...
**Evidence:**
- 5 skill folders (`jodex-gen-brd-prd`, `jodex-gen-prd`, `jodex-gen-prd-task`, `jodex-gen-tds`, `jodex-gen-prd-lite`) with 19 markdown files across nested subdirectories
...
- Automate deployment of all `.md` files from `./skills/jodex-*` to `~/.claude/skills/` in a single command
...
**I want** a single command that copies all `.md` files from `./skills/jodex-*` to `~/.claude/skills/`
**So that** I can deploy updated skill definitions without manual file management

**Acceptance Criteria:**
- [ ] Script copies every `.md` file found under `./skills/jodex-*/` (including nested subdirectories like `ai_docs/`, `prd_examples/`, `example_tech_spec_md/`)
...
- FR-3: Script must recursively find all `*.md` files within matched skill directories *(Supports US-001)*
...
- **Performance:** Script completes in <5 seconds for the current set of 19 `.md` files.
...
- No deployment of non-`.md` files (`.doc`, `.docx`, etc.) — only markdown files are in scope
- No deployment of skill folders outside the `jodex-*` pattern (e.g., `CLAUDE/`, `AGENTS/`)
